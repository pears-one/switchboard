...
from rotated_tile import RotatedTile
...
class Board:
    ...
    def from_config(filename, tile_repo: TileRepo):
        ...
        for tile_config in board_config:
            pos = TilePosition(tile_config[0], tile_config[1])
            tile_id = TileID(tile_config[2], tile_config[3])
            tile_dictionary[tuple(pos)] = RotatedTile(tile_repo.get_tile(tile_id), tile_config[4])
    ...
    def move_tile(self, from_pos, to_pos, rotation):
        ...
        tile_to_move = self.get_tile_at(from_pos).get_underlying_tile()
